...
class Trinity_hunter_rev:
    def __init__(self, sec_range):
        self.sec_range = sec_range
    
    """
    Calculates the coeficients for 2 lines. 
    """
    def setup(self):
        co = math.tan(math.pi / 6) * self.center[0]
        self.bp = self.center[1] - co
        self.ap = (self.center[1] - self.bp)/(self.center[0])
        self.bn = self.center[1] + co
        self.an = - self.ap
    ...
    def init_condition(self, candidate):
        return math.fabs((self.ap * candidate[0] + self.bp) - candidate[1]) *\
            math.sin(math.pi / 6) <= self.sec_range and candidate[0] >\
                self.center[0] and candidate[1] > self.center[1]
    ...
    def mirror_condition(self, candidate):
        return math.fabs((self.an * candidate[0] + self.bn) - candidate[1]) *\
            math.sin(math.pi / 6) <= self.sec_range and candidate[0] <\
                self.center[0] and candidate[1] > self.center[1]
    ...
    def top_condition(self, candidate):
        return math.fabs(candidate[0] - self.center[0]) <= self.sec_range\
